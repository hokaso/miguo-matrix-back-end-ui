<template>
  <div class="app-container">
    ...
    <el-table-editabled v-model="tableData" :columns="['swiperName', 'groupProfile', 'swiperPic', 'activityName', 'activityId']" ref="editTable">
      <el-table
        :key="tableKey"
      ...
      >
        ...
        <el-table-column label="轮播图名称" min-width="80px" align="center">
          <template slot-scope="{row}">
        ...
        <el-table-column label="轮播图" min-width="100px" align="center">
          <template slot-scope="{row}">
            <el-button v-show="!row.swiperPic" type="primary" size="mini" @click="imageCropperShow(row)">
              上传图片
            </el-button>
            <el-button v-show="row.swiperPic" type="primary" size="mini" @click="imageCropperShow(row)">
              更新图片
            </el-button>
            <el-button v-show="row.swiperPic" type="primary" size="mini" @click="picShow(row.swiperPic)">
              预览
            </el-button>
          </template>
        </el-table-column>
        <el-table-column label="轮播图对应活动" min-width="100px" align="center">
          <template slot-scope="{row}">
    ...
    <el-dialog title="图片预览" :visible.sync="picVisible" width="960px" center>
...
<script>
    ...
    import MpSwiperApi from "@/api/miniprogram/MpSwiperApi";
    ...
    export default {
        ...
        methods: {
            ...
            cropUploadSuccess(jsonData, field){
                this.temp.swiperPic = jsonData.data
                const tempData = Object.assign({}, this.temp)
                MpSwiperApi.update(tempData).then(() => {
                    this.$notify({
                        title: 'success',
                        message: '图片上传成功',
                        type: 'success',
            ...
            getList() {
                ...
                MpSwiperApi.findAllByKeywords(this.listQuery).then(data => {
            ...
            // 删除选中数据
            deleteGroup () {
                MpSwiperApi.del(this.multipleSelection).then(data => {
                    console.log(data)
                    this.getList()
                })
            },
